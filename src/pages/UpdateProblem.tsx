...
import { ProblemApiResponse, ProblemFormData } from "../types/types";

const UpdateProblem = () => {
  const { problemId } = useParams(); // Type the useParams hook
  ...
  useEffect(() => {
    async function fetchProblem() {
      try {
        ...
        const response = await axios.get<ProblemApiResponse>(problemURL);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const problemURL = `${serverConfig.PROBLEM_SERVICE_HOSTNAME}/api/v1/problems/${problemId}`;
      const response = await axios.put(problemURL, formData);
